...
import { useSlotState,useSlotActions } from "./SlotProvider";
...
const Appointment = (props) => {
  ...
  const {slot,slots} = useSlotState()
  const {setSlotAction,setSlotsAction} = useSlotActions()

  useEffect(()=>{
    setSlotAction("Hi there")

    setTimeout(() => {
    setSlotsAction([1,2,3,4])
